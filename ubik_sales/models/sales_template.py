...
class SaleOrderLine(models.Model):
    ...
    ############# CUSTOM FIELDS FOR EXPORT TYPE OF SALE ORDER: starts #############
    hsn_code = fields.Char(string='HSN Code', related='product_id.l10n_in_hsn_code')
    dbk_code = fields.Char(string='DBK Code', related='product_id.dbk_code')
    net_weight_per_box = fields.Float(string="Net Weight (kg/box)", related='product_id.net_weight_per_box', store=True)
    gross_weight_per_box = fields.Float(string="Gross Weight (kg/box)", related='product_id.gross_weight_per_box', store=True)
    box_dimensions = fields.Char(string="Dimensions (mm)", related='product_id.box_dimensions', store=True)
    best_before = fields.Integer(
        string="Best Before (Months)",
        compute="_compute_best_before",
        store=True
    )

    @api.depends('product_id.use_time')
    def _compute_best_before(self):
        for line in self:
            if line.product_id.use_time:
                # Convert days → months and round
               line.best_before = math.ceil(line.product_id.use_time / 30)
            else:
                line.best_before = 0

    # From stock.move
    no_of_boxes = fields.Integer(string="No. of Boxes", compute="_compute_delivery_details", store=True)
    marks_nos = fields.Char(string="Marks & Nos", compute="_compute_delivery_details", store=True)

    # From stock.move.line
    lot_id = fields.Many2one('stock.lot', string="Lot/Serial No", compute="_compute_delivery_details", store=True)
    expiration_date = fields.Date(string="Expiration Date", compute="_compute_delivery_details", store=True)

    @api.depends(
    'move_ids.no_of_boxes',
    'move_ids.marks_nos',
    'move_ids.move_line_ids.lot_id',
    'move_ids.move_line_ids.lot_id.expiration_date',
    'move_ids.state'
)
    def _compute_delivery_details(self):
        for line in self:
            moves = line.move_ids.filtered(lambda m: m.state not in ('cancel'))

            # Default values
            line.no_of_boxes = 0
            line.marks_nos = False
            line.lot_id = False
            ...
            # Take first move (you can customize logic)
            move = moves[0]

            # From stock.move
            line.no_of_boxes = move.no_of_boxes
            line.marks_nos = move.marks_nos
...
class StockMove(models.Model):
    ...
    no_of_boxes = fields.Integer(string="No. of Boxes")
    marks_nos = fields.Char(string="Marks & Nos")
    ############# CUSTOM FIELDS FOR EXPORT TYPE OF SALE ORDER: ends #############

    ######## Code to open Open: Stock move wizard: starts ################
    ###******DO NOT FORGET TO UPDATE ACCESS RIGHTS FOR THE TWO MODELS BELOW IN THE CSV FILE*********###
#     access_so_move_lot_wizard,access.so.move.lot.wizard,ubik_sales.model_so_move_lot_wizard,base.group_user,1,1,1,1
# access_so_move_lot_wizard_line,access.so.move.lot.wizard.line,ubik_sales.model_so_move_lot_wizard_line,base.group_user,1,1,1,1
 ###******DO NOT FORGET TO UPDATE ACCESS RIGHTS FOR THE TWO MODELS BELOW IN THE CSV FILE*********###
...
class SaleOrder(models.Model):
    ...
    consignee_type = fields.Selection([
        ('same', 'Same as Customer'),
        ('different', 'Different Consignee')
    ], string="Consignee Type", default='same')
    ...
    def _onchange_partner_id_consignee(self):
        for rec in self:
            if rec.consignee_type == 'same':
                rec.consignee_id = rec.partner_id

    delivery = fields.Char(string='Delivery')
    end_use_code = fields.Char(string='End Use Code')
    pre_carraige = fields.Char(string='Pre Carriage')
    vessel_number = fields.Char(string='Vessel/Container Number')
    place_of_receipt = fields.Char(string='Place of Receipt by Pre Carrier')
